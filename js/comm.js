...
COM.biz = (function() {
   ...
   */
	...
	var loader = function(status, tips) {
		...
		if(status == "show" && loaderContainer) {
...
COM.post = function(url, options) {
	...
	headerData["action"] = url.replace("/appapi/", "");
	headerData["app"] = "smartsolarapp";
	headerData["user"] = encodeURI(localStorage.getItem("user")).toLowerCase();
	var needMd5 = "app" + headerData["app"] + "scretesmartsolarapp_2017user" + headerData["user"] + "action" + headerData["action"] + "timestamp" + headerData["timestamp"];
	headerData["sign"] = hex_md5(needMd5);
	headerData["Content-Type"] = "application/json";
	COM.console(JSON.stringify(headerData));
	...
	COM.console("请求的url: " + url);
	COM.console("请求的参数: " + JSON.stringify(setting.data))
	mui.ajax(url, {
		headers: headerData,
		data: setting.data,
		dataType: 'json',
		...
		success: function(res) {
			COM.console("appapi/" + headerData["action"] + " 异步请求已经成功发送，并接收到返回。" + JSON.stringify(res));
			if(res.Code === 0) {
